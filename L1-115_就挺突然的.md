...
本题属于就挺突然的题型，核心在于计算寿命 $d=B-A$ 并分类：$d>250$ 输出 jiu ting tu ran de...，$d\le0$ 输出 hai sheng ma?，否则 nin tai cong ming le!。输入规模较小，可直接按题意模拟，无需复杂数据结构。
...
2. **核心计算**：计算寿命 $d=B-A$ 并分类：$d>250$ 输出 jiu ting tu ran de...，$d\le0$ 输出 hai sheng ma?，否则 nin tai cong ming le!，在循环体内逐项累加、比较或变换（如代码中的 `for` 循环体），维护中间变量（如求和、计数、查表）。
