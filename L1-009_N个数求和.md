...
本题的核心逻辑为：每次按 $\frac{a}{b}+\frac{c}{d}=\frac{ad+bc}{bd}$ 合并分数，并用最大公约数约分。最后用整数除法拆出整数部分和真分数部分。根据题意将输入转化为可计算的模型后，直接按规则求解即可。
...
2. **核心处理**：每次按 $\frac{a}{b}+\frac{c}{d}=\frac{ad+bc}{bd}$ 合并分数，并用最大公约数约分。最后用整数除法拆出整数部分和真分数部分。
