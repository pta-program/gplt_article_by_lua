...
本题要求根据给定的输入数据完成相应的判定或计算任务，以“L2-057_姥姥改作业”为核心，需在约束条件下构造出满足题意的结果。以 Lua 实现为参照，其实现原理概括为：两堆栈模拟姥姥改作业，左堆存 $c_i>T$ 者，右堆存已批改作业，按 $T=\lfloor\text{平均值}\rfloor$ 迭代，按 LIFO 顺序取左堆。
...
3. 核心逻辑处理：依据“两堆栈模拟姥姥改作业，左堆存 $c_i>T$ 者，右堆存已批改作业，按 $T=\lfloor\text{平均值}\rfloor$ …”的原理进行迭代计算，完成题面要求的判定、统计或构造任务，并在循环中处理相等、冲突等特殊分支。
